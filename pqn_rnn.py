...
class RNNQNetwork(nn.Module):
    ...
    def __call__(self, hidden, x, done, last_action, train: bool = False):
        ...
        print(f"after RNN x.shape: {x.shape}" )
        print(f"after RNN hidden.shape: {hidden.shape}" )
...
def make_train(config):
    ...
    env, env_params = popjym.make(config["ENV_NAME"], partial_observable=True)
...
if __name__ == "__main__":
    ...
    "HYP_TUNE": False,
    "ENTITY": "",
    "PROJECT": "flax_pqn_rnn",
    "WANDB_MODE": "online",
    "SEED": 0,
    "NUM_SEEDS": 1,

    # env specific
    "ENV_NAME": "CartPoleHard", # should work also for Acrobot-v1 but might need some tuning
    "ENV_KWARGS": {},
